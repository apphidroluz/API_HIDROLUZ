...
public class XML_TABController {
	...
	private final SimpleDateFormat dateFormatentra = new SimpleDateFormat("yyyy-MM-dd ");
	private final SimpleDateFormat dateFormatsai = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
...
//	private XML_TabService xml_TabService;
	...
	public ResponseEntity<Response<XML_TABDto>> buscarConceAndData(
			@PathParam("buscar") @Valid @RequestBody XML_TABDto xml_TABDto, BindingResult result) throws ParseException{
		Response<XML_TABDto> response = new Response<XML_TABDto>();

		Date date_info = this.dateFormatentra.parse(xml_TABDto.getData());

		XML_TAB xmlDto = this.xmlRepository.findByConcentradorAndData(xml_TABDto.getConcentrador(), date_info);
	...
	public ResponseEntity<Response<XML_TABDto>> buscarNumHidroAndData(@Valid @RequestBody XML_TABDto xml_TABDto,
	...
	private XML_TABDto converterXMLDTO(XML_TAB tab) {
		...
		dto.setData(this.dateFormatsai.format(tab.getData()));
	...
	private XML_TAB converterDTOparaXMl(XML_TABDto dto, BindingResult result) throws ParseException {
		...
		tab.setData(this.dateFormatsai.parse(dto.getData()));
